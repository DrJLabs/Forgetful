...
def validate_basic_config(config: Config) -> ValidationResult:
    ...
    if config.DATABASE_PORT < 1024 or config.DATABASE_PORT > 65535:
        result.add_error(f"DATABASE_PORT {config.DATABASE_PORT} is not in valid range (1024-65535)")
    
    if config.NEO4J_PORT < 1024 or config.NEO4J_PORT > 65535:
        result.add_error(f"NEO4J_PORT {config.NEO4J_PORT} is not in valid range (1024-65535)")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                cur.execute("SELECT version() FROM pg_extension WHERE extname = 'vector';")
    ...
    try:
        ...
        try:
            ...
            if config.OPENAI_MODEL in models:
                result.add_success(f"OpenAI API connection successful, model {config.OPENAI_MODEL} available")
            else:
                result.add_warning(f"OpenAI API connected but model {config.OPENAI_MODEL} not found in available models")
            
            if config.OPENAI_EMBEDDING_MODEL in models:
                result.add_success(f"Embedding model {config.OPENAI_EMBEDDING_MODEL} available")
            else:
                result.add_warning(f"Embedding model {config.OPENAI_EMBEDDING_MODEL} not found")
        ...
        for service_name, url in service_urls:
            try:
                ...
                if response.status_code < 500:
                    result.add_success(f"{service_name} is accessible ({response.status_code})")
                else:
                    result.add_warning(f"{service_name} returned {response.status_code}")
    ...
    for path, path_type in paths_to_check:
        ...
        if path_type == 'directory':
            ...
            if full_path.exists() and full_path.is_file():
                ...
                if path == '.env':
                    result.add_warning(f"File {path} does not exist (use env.template to create it)")
                else:
