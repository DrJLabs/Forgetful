...
def benchmark_vector_operations(
    ...
    # Create test table
    table_name = 'benchmark_vectors_pgvector' if use_pgvector else 'benchmark_vectors_string'
    ...
    print(f"✅ Insertion: {insert_time:.2f}s ({results['insert_rate']:.1f} vectors/sec)")
    ...
    print(f"✅ Search: {avg_search_time*1000:.2f}ms avg ({results['search_rate']:.1f} searches/sec)")
    ...
    print(f"✅ Batch insert: {batch_time:.2f}s ({results['batch_rate']:.1f} vectors/sec)")
