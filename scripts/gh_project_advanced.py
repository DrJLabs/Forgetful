...
class GitHubProjectManager:
    ...
    def generate_project_analytics(self) -> Dict:
        ...
        for item in items:
            for field_value in item['fieldValues']['nodes']:
                ...
                if field_name:
                    ...
                    value = field_value.get('name') or field_value.get('text', 'Unassigned')
                    field_distributions[field_name][value] = field_distributions[field_name].get(value, 0) + 1
        ...
        if total_items > 0:
            ...
            analytics["health_metrics"] = {
                ...
                "assigned_rate": round(((total_items - field_distributions.get('Status', {}).get('Unassigned', 0)) / total_items) * 100, 2)
            }
            
            # Priority distribution analysis
            priority_dist = field_distributions.get('Priority', {})
            critical_high = priority_dist.get('Critical', 0) + priority_dist.get('High', 0)
            analytics["health_metrics"]["priority_focus"] = round((critical_high / total_items) * 100, 2)
    ...
    def create_sprint_board_view(self, sprint_name: str) -> Dict:
        ...
        for item in items:
            for field_value in item['fieldValues']['nodes']:
                if (field_value.get('field', {}).get('name') == 'Sprint' and 
                    field_value.get('name', '').lower() == sprint_name.lower()):
        ...
        for item in sprint_items:
            ...
            item_info = {
                ...
                "component": component,
                "priority": priority,
                "assignees": [a['login'] for a in item['content']['assignees']['nodes']] if item['content'] and item['content']['assignees'] else []
            }
    ...
    def bulk_update_items(self, updates: List[Dict]) -> Dict:
        ...
        for update in updates:
            ...
            for field_name, value in field_updates.items():
                if field_name in self.fields and value in self.options.get(field_name, {}):
    ...
    def update_item_field(self, item_id: str, field_id: str, option_id: str) -> bool:
        ...
        return bool(result and 'data' in result and result['data']['updateProjectV2ItemFieldValue'])
    ...
    def generate_velocity_report(self, weeks: int = 4) -> Dict:
        ...
        for item in items:
            ...
            if status == 'Done':
                completed_items.append({
                    "title": item['content']['title'] if item['content'] else "Draft",
                    "effort": effort,
                    "completed_date": item['content']['updatedAt'] if item['content'] else item['createdAt']
                })
        ...
        total_points = sum(effort_points.get(item['effort'], 3) for item in completed_items)
...
def main():
    parser = argparse.ArgumentParser(description="Advanced GitHub Projects v2 GraphQL Manager")
    parser.add_argument("command", choices=[
        "analytics", "sprint-board", "velocity", "overview", "health-check"
    ], help="Command to execute")
    parser.add_argument("--sprint", help="Sprint name for sprint-board command")
    parser.add_argument("--weeks", type=int, default=4, help="Number of weeks for velocity report")
    ...
    try:
        if args.command == "analytics":
            ...
            result = raw_data['data']['organization']['projectV2'] if raw_data and 'data' in raw_data else {}
...
def print_velocity_report(velocity: Dict):
    ...
    for item in velocity.get('items', []):
        effort_emoji = {"XS (1 hour)": "🟢", "S (2-4 hours)": "🟡", "M (1-2 days)": "🟠"}.get(item['effort'], "🔴")
