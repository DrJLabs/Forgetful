...
from mem0.configs.coding_config import CodingMemoryConfig, CodingFactExtractor, create_coding_optimized_config
from mem0.memory.coding_memory import CodingMemory, AsyncCodingMemory
from mem0.memory.enhanced_deduplication import EnhancedDeduplicator, AutonomousDeduplicationManager
...
class TestCodingMemory:
    ...
    def setup_method(self):
        ...
        # Create patches
        self.embedding_patch = patch('mem0.utils.factory.EmbedderFactory.create', return_value=self.mock_embedding_model)
        self.vector_patch = patch('mem0.utils.factory.VectorStoreFactory.create', return_value=self.mock_vector_store)
        self.llm_patch = patch('mem0.utils.factory.LlmFactory.create', return_value=self.mock_llm)
        self.db_patch = patch('mem0.memory.storage.SQLiteManager', return_value=self.mock_db)
    ...
    def test_rank_coding_results(self):
        ...
        ranked_results = memory._rank_coding_results(results, "memory fix", "bug_fixing")
...
class TestEnhancedDeduplication:
        ...
        existing_memories = [
            {
                'id': '1',
                'memory': 'Fixed memory leak in React component',
                'metadata': {'category': 'bug_fix', 'created_at': '2025-01-01T10:00:00Z'}
            }
        ]
    ...
    def test_should_deduplicate_different_facts(self):
        ...
        metadata = {'category': 'code_implementation', 'created_at': '2025-01-01T10:01:00Z'}
    ...
    def test_progressive_refinement_detection(self):
        ...
        existing_memory = {
            'memory': 'Database query performance issue',
            'metadata': {}
        }
        metadata = {}
...
class TestMemoryCategorization:
    ...
    def test_categorize_memory_bug_fix(self):
        ...
        memory_content = "Fixed critical bug in authentication system causing login failures"
        metadata = {"error_related": True}
    ...
    def test_auto_categorizer(self):
        ...
        assert result["auto_action"] in ["auto_categorize", "suggest_with_review", "manual_categorization"]
...
class TestMetadataTagging:
    ...
    def test_auto_tagging_manager(self):
        ...
        assert result["auto_tagging_decision"] in [
            "auto_tag_all", "auto_tag_high_confidence", 
            "suggest_with_review", "manual_tagging_required"
        ]
