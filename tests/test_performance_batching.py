...
class TestBatchingPerformance:
    ...
    def generate_batch_requests(self, count: int, priority: BatchPriority = BatchPriority.NORMAL):
    ...
    async def test_batch_processor_performance(self, batch_processor):
        ...
        try:
            ...
            assert len(successful_results) == len(requests), f"Expected {len(requests)} results, got {len(successful_results)}"
            
            # Check batch processor stats
            stats = batch_processor.get_stats()
            
            assert stats['batches_processed'] > 0, "Should have processed batches"
            assert stats['requests_processed'] == len(requests), f"Expected {len(requests)} requests processed"
            assert stats['success_rate'] > 0.95, f"Success rate too low: {stats['success_rate']:.3f}"
            
            print(f"Batch Processor Performance: Time={total_time:.3f}s, Batches={stats['batches_processed']}, Success Rate={stats['success_rate']:.3f}")
    ...
    async def test_priority_batching_performance(self, batch_processor):
        ...
        try:
            ...
            all_requests = urgent_requests + high_requests + normal_requests + low_requests
            ...
            successful_results = [r for r in results if not isinstance(r, Exception)]
            assert len(successful_results) == len(all_requests), f"Expected {len(all_requests)} results"
            
            # Check stats
            stats = batch_processor.get_stats()
            assert stats['requests_processed'] == len(all_requests), "Should process all requests"
            
            print(f"Priority Batching Performance: Time={total_time:.3f}s, Requests={len(all_requests)}")
    ...
    async def test_memory_write_batcher_performance(self):
        ...
        with patch('shared.batching.global_pool_manager') as mock_manager:
            ...
            try:
                ...
                # Performance assertions
                assert total_time < 10.0, f"Memory write batching too slow: {total_time:.3f}s"
                
                # Verify results
                successful_results = [r for r in results if not isinstance(r, Exception)]
                assert len(successful_results) == len(test_data), f"Expected {len(test_data)} results"
                
                # Check stats
                stats = batcher.get_stats()
                assert stats['requests_processed'] == len(test_data), "Should process all writes"
                
                print(f"Memory Write Batcher Performance: Time={total_time:.3f}s, Writes={len(test_data)}, Success Rate={stats['success_rate']:.3f}")
            ...
            try:
                ...
                for i in range(20):
                    test_searches.append({
                        ...
                        'query_embedding': [random.random() for _ in range(10)],  # 10-dim vector
                        'limit': 5,
                        'filters': {'type': 'test'}
                    })
                ...
                # Performance assertions
                assert total_time < 8.0, f"Vector search batching too slow: {total_time:.3f}s"
                
                # Verify results
                successful_results = [r for r in results if not isinstance(r, Exception)]
                assert len(successful_results) == len(test_searches), f"Expected {len(test_searches)} results"
                
                # Check stats
                stats = batcher.get_stats()
                assert stats['requests_processed'] == len(test_searches), "Should process all searches"
                
                print(f"Vector Search Batcher Performance: Time={total_time:.3f}s, Searches={len(test_searches)}, Success Rate={stats['success_rate']:.3f}")
    ...
    async def test_graph_query_batcher_performance(self):
        ...
        with patch('shared.batching.global_pool_manager') as mock_manager:
            ...
            try:
                ...
                # Performance assertions
                assert total_time < 8.0, f"Graph query batching too slow: {total_time:.3f}s"
                
                # Verify results
                successful_results = [r for r in results if not isinstance(r, Exception)]
                assert len(successful_results) == len(test_queries), f"Expected {len(test_queries)} results"
                
                # Check stats
                stats = batcher.get_stats()
                assert stats['requests_processed'] == len(test_queries), "Should process all queries"
                
                print(f"Graph Query Batcher Performance: Time={total_time:.3f}s, Queries={len(test_queries)}, Success Rate={stats['success_rate']:.3f}")
    ...
    async def test_batching_manager_performance(self):
        ...
        mock_neo4j_pool.session.return_value.__aenter__.return_value = mock_neo4j_session
        ...
        with patch('shared.batching.global_pool_manager') as mock_manager:
            ...
            try:
                ...
                # Performance assertions
                assert total_time < 15.0, f"Batching manager too slow: {total_time:.3f}s"
                
                # Verify results
                successful_results = [r for r in results if not isinstance(r, Exception)]
                assert len(successful_results) == len(operations), f"Expected {len(operations)} results"
                
                # Check comprehensive stats
                stats = manager.get_comprehensive_stats()
                
                assert 'memory_write_batcher' in stats, "Should have memory write stats"
                assert 'vector_search_batcher' in stats, "Should have vector search stats"
                assert 'graph_query_batcher' in stats, "Should have graph query stats"
                
                print(f"Batching Manager Performance: Time={total_time:.3f}s, Operations={len(operations)}")
    ...
    async def test_batch_timeout_handling(self, batch_processor):
        ...
        slow_processor_instance = BatchProcessor("slow_processor", slow_processor, config)
        ...
        try:
            ...
            print(f"Timeout Handling Performance: Time={total_time:.3f}s, Timeouts={len(timeout_results)}")
    ...
    async def test_batch_error_recovery(self, batch_processor):
        ...
        failing_processor_instance = BatchProcessor("failing_processor", failing_processor, config)
        ...
        try:
            ...
            tasks = []
            for request in requests:
                task = asyncio.create_task(failing_processor_instance.add_request(request))
                tasks.append(task)
            ...
            successful_results = [r for r in results if not isinstance(r, Exception)]
            assert len(successful_results) == len(requests), f"Expected all requests to eventually succeed"
            
            # Check stats
            stats = failing_processor_instance.get_stats()
            assert stats['retry_count'] > 0, "Should have recorded retries"
            
            print(f"Error Recovery Performance: Time={total_time:.3f}s, Retries={stats['retry_count']}")
    ...
    def test_batch_metrics_performance(self):
        ...
        assert stats['batches_processed'] == 1000, f"Expected 1000 batches, got {stats['batches_processed']}"
        assert stats['requests_processed'] == 10000, f"Expected 10000 requests, got {stats['requests_processed']}"
        assert stats['requests_failed'] == 100, f"Expected 100 failures, got {stats['requests_failed']}"
        assert stats['flush_count'] == 10, f"Expected 10 flushes, got {stats['flush_count']}"
        assert stats['timeout_count'] == 20, f"Expected 20 timeouts, got {stats['timeout_count']}"
        assert stats['retry_count'] == 20, f"Expected 20 retries, got {stats['retry_count']}"
        
        # Check calculated metrics
        assert stats['success_rate'] > 0.99, f"Success rate too low: {stats['success_rate']:.3f}"
        assert stats['average_batch_size'] == 10.0, f"Average batch size incorrect: {stats['average_batch_size']}"
        
        print(f"Batch Metrics Performance: Recording={metrics_time:.3f}s, Stats={stats_time:.3f}s")
