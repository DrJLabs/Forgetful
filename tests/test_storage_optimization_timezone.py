...
class TestTimezoneEdgeCases:
    ...
    def create_test_memory(
        self, created_at=None, last_accessed=None, category="general"
    ):
    ...
    def test_mixed_timezone_scenarios(self):
        ...
        memories = [
            ...
            self.create_test_memory(
                created_at="2024-01-01T12:00:00Z", category="testing"
            ),
            # UTC with +00:00 suffix
            self.create_test_memory(
                created_at="2024-01-01T12:00:00+00:00", category="testing"
            ),
            # Different timezone
            self.create_test_memory(
                created_at="2024-01-01T12:00:00+05:00", category="testing"
            ),
            # Recent memory (should not be purged)
            self.create_test_memory(
                created_at=create_memory_timestamp(), category="testing"
            ),
        ]
    ...
    def test_dst_transition_scenarios(self):
        ...
        with freeze_time("2024-03-10T12:00:00Z"):
            ...
            purged = self.storage_manager._purge_category_memories(memories, 1, policy)
        ...
        with freeze_time("2024-06-01T12:00:00Z"):
            for case in test_cases:
                memory = self.create_test_memory(
                    created_at=case["timestamp"], last_accessed=case["timestamp"]
                )
                ...
                assert (
                    case["expected_range"][0]
                    <= recency_factor
                    <= case["expected_range"][1]
                ), (
...
class TestStorageOptimizationIntegration:
    ...
    def create_memory_dataset(self, count=50):
        ...
        for i in range(count):
            ...
            memory = {
                "id": str(uuid.uuid4()),
                "memory": f"Test memory {i} with content length variation"
                * (i % 5 + 1),
                "metadata": {
...
class TestTimezoneRegressionTests:
    ...
    def setup_method(self):
        """Setup test fixtures."""
        self.config = {"max_memories_total": 1000, "max_memories_per_category": 200}
        self.storage_manager = IntelligentStorageManager(self.config)
    ...
    def test_original_timezone_bug_regression(self):
        ...
        try:
            purged = self.storage_manager._purge_category_memories([memory], 1, policy)
            ...
            if len(purged) > 0:
                ...
                assert (
                    memory["metadata"]["success_rate"] < 0.3
                    or memory["metadata"]["access_count"] < 2
                )
...
class TestPerformanceRegression:
    ...
    def setup_method(self):
        """Setup test fixtures."""
        self.config = {"max_memories_total": 10000, "max_memories_per_category": 2000}
        self.storage_manager = IntelligentStorageManager(self.config)
        ...
        for i in range(count):
            ...
            memory = {
                ...
                "metadata": {
                    "created_at": memory_time.isoformat(),
                    "last_accessed": memory_time.isoformat(),
                    "category": ["general", "testing", "debugging", "performance"][
                        i % 4
                    ],
                    "access_count": i % 20,
                    "success_rate": 0.5 + (i % 5) * 0.1,
