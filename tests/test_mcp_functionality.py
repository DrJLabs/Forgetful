            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url) as response:
                    ...
                    self.test_results.append(
                        ("Endpoint Access", True, f"Status: {status}")
                    )
                    return True
            ...
            async with aiohttp.ClientSession() as session:
                ...
                test_memory = {
                    "messages": [
                        ...
                        {
                            "role": "assistant",
                            "content": f"Testing MCP server at {datetime.now().isoformat()}",
                        },
                    ],
                ...
                async with session.post(create_url, json=test_memory) as response:
                    if response.status == 200:
                        ...
                        self.test_results.append(
                            ("API Memory Creation", True, "Success")
                        )
                        ...
                        async with session.post(
                            search_url, json=search_data
                        ) as search_response:
                            if search_response.status == 200:
                                search_result = await search_response.json()
                                memories_found = len(search_result.get("results", []))
                                print(
                                    f"✅ Search via API: Found {memories_found} memories"
                                )
                                self.test_results.append(
                                    (
                                        "API Search",
                                        True,
                                        f"Found {memories_found} memories",
                                    )
                                )
                            else:
                        ...
                        self.test_results.append(
                            ("API Memory Creation", False, f"Status: {response.status}")
                        )
                        ...
                        async with session.post(
                            ...
                            if search_response.status == 200:
                                ...
                                if result.get("results"):
                                    ...
                                    self.test_results.append(
                                        ("Backend Consistency", True, "Verified")
                                    )
                                else:
