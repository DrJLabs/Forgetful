...
class TestCachePerformance:
    ...
    def generate_test_data(self, size: int = 100) -> list:
        ...
        return [
            {
                'key': f"test_key_{i}",
                'value': ''.join(random.choices(string.ascii_letters, k=1024)),  # 1KB values
    ...
    async def test_l1_cache_performance(self, l1_cache):
        ...
        # Hit rate assertions
        assert hit_count == len(test_data), f"L1 cache hit count mismatch: {hit_count}/{len(test_data)}"
        assert stats['hit_rate'] > 0.98, f"L1 cache hit rate too low: {stats['hit_rate']:.3f}"
        
        # Memory usage assertions
        assert stats['memory_usage_mb'] < 100, f"L1 cache memory usage too high: {stats['memory_usage_mb']:.1f}MB"
        
        print(f"L1 Cache Performance: Write={write_time:.3f}s, Read={read_time:.3f}s, Hit Rate={stats['hit_rate']:.3f}")
    ...
    async def test_l2_cache_performance(self, l2_cache):
        ...
        assert hit_count >= len(test_data) * 0.8, f"L2 cache hit count too low: {hit_count}/{len(test_data)}"
        
        print(f"L2 Cache Performance: Write={write_time:.3f}s, Read={read_time:.3f}s, Hits={hit_count}/{len(test_data)}")
    ...
    async def test_l3_cache_performance(self, l3_cache):
        ...
        # Hit rate assertions
        assert hit_count == len(test_data), f"L3 cache hit count mismatch: {hit_count}/{len(test_data)}"
        assert stats['hit_rate'] > 0.98, f"L3 cache hit rate too low: {stats['hit_rate']:.3f}"
        
        print(f"L3 Cache Performance: Write={write_time:.3f}s, Read={read_time:.3f}s, Hit Rate={stats['hit_rate']:.3f}")
    ...
    async def test_multi_layer_cache_performance(self, multi_layer_cache):
        ...
        # Performance assertions
        assert write_time < 10.0, f"Multi-layer cache write time too slow: {write_time:.3f}s"
        assert read_time < 3.0, f"Multi-layer cache read time too slow: {read_time:.3f}s"
        assert total_time < 15.0, f"Multi-layer cache total time too slow: {total_time:.3f}s"
        
        # Hit rate assertions
        assert hit_count >= len(test_data) * 0.9, f"Multi-layer cache hit count too low: {hit_count}/{len(test_data)}"
        assert stats['overall_hit_rate'] > 0.85, f"Multi-layer cache hit rate too low: {stats['overall_hit_rate']:.3f}"
        
        print(f"Multi-layer Cache Performance: Write={write_time:.3f}s, Read={read_time:.3f}s, Hit Rate={stats['overall_hit_rate']:.3f}")
    ...
    async def test_cache_hit_miss_ratios_under_load(self, l1_cache):
        ...
        assert read_hit_rate > 0.8, f"Cache hit rate too low under load: {read_hit_rate:.3f}"
        assert stats['hit_rate'] > 0.7, f"Overall hit rate too low: {stats['hit_rate']:.3f}"
        
        print(f"Load Test: Read Hit Rate={read_hit_rate:.3f}, Overall Hit Rate={stats['hit_rate']:.3f}")
    ...
    async def test_concurrent_cache_operations(self, multi_layer_cache):
        ...
        print(f"Concurrent Performance: Write={write_time:.3f}s, Read={read_time:.3f}s, Hit Rate={hit_rate:.3f}")
    ...
    async def test_memory_usage_and_cleanup(self, l1_cache):
        ...
        large_data = []
        for i in range(100):
            large_value = ''.join(random.choices(string.ascii_letters, k=10240))  # 10KB values
        ...
        print(f"Memory Test: Write={write_time:.3f}s, Memory Increase={memory_increase:.1f}MB, Growth={memory_growth:.1f}MB")
    ...
    async def test_cache_invalidation_performance(self, multi_layer_cache):
        ...
        # Performance assertions
        assert invalidation_time < 2.0, f"Invalidation time too slow: {invalidation_time:.3f}s"
        
        # Correctness assertions
        user_5_items = len([item for item in test_data if item['user_id'] == "user_5"])
        assert invalidated_count >= user_5_items * 0.8, f"Invalidation incomplete: {invalidated_count}/{user_5_items}"
        
        print(f"Invalidation Performance: Time={invalidation_time:.3f}s, Invalidated={invalidated_count}/{user_5_items}")
    ...
    def test_configuration_validation(self):
        ...
        with pytest.raises(ValueError, match="L1 TTL must be between 60 and 86400 seconds"):
        ...
        with pytest.raises(ValueError, match="L3 max prepared statements must be between"):
