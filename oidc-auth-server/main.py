...
async def openid_configuration():
    ...
    return {
        ...
        "code_challenge_methods_supported": [
            "S256",
            "plain",
        ],  # Advertise both supported methods
    }
...
@app.post("/auth/token")
@limiter.limit("10/minute")
async def token_endpoint(request: Request, token_request: TokenRequest):
    """
...
async def handle_authorization_code_grant(token_request: TokenRequest):
    ...
    if original_request.get("code_challenge"):
        ...
        if (
            len(token_request.code_verifier) < 43
            or len(token_request.code_verifier) > 128
        ):
            return JSONResponse(
                status_code=400,
                content={
        ...
        if challenge_method == "S256":
            ...
            verifier_hash = hashlib.sha256(
                token_request.code_verifier.encode()
            ).digest()
            computed_challenge = (
    ...
    payload = {
        ...
        "iss": BASE_URL,
        "aud": code_data["original_request"].get("resource")
        or "https://api.openmemory.io",
        "iat": now,
    ...
    access_token = jwt.encode(
        payload, private_key_pem, algorithm=JWT_ALGORITHM, headers={"kid": KEY_ID}
    )
    ...
    refresh_tokens[refresh_token] = {
        ...
        "scope": code_data["original_request"]["scope"],
        "audience": code_data["original_request"].get("resource")
        or "https://api.openmemory.io",
        "user_info": user_info,
        "created_at": datetime.utcnow(),
...
async def userinfo(request: Request):
    ...
    try:
        ...
        payload = jwt.decode(
            token, public_key_pem, algorithms=[JWT_ALGORITHM], issuer=BASE_URL
        )
        return {
            "sub": payload["sub"],
            "email": payload["email"],
