...
echo -e "\n2. Database status:"
docker exec postgres-mem0 psql -U drj -d mem0 -t -c "SELECT 'memories table:', COUNT(*) FROM memories;"
...
docker exec postgres-mem0 psql -U drj -d mem0 -c "SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename LIKE '%memor%';" | grep -v tablename | grep -v "^-" | sort
