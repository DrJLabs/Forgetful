...
class MemoryTester:
    ...
    def test_openmemory_create_memory(self) -> TestResult:
        ...
        test_data = {
            ...
            "metadata": {
                "test": "openmemory_creation",
                "timestamp": datetime.now().isoformat(),
            },
        }

        response = requests.post(
            f"{self.openmemory_base_url}/api/v1/memories/", json=test_data
        )
            ...
            try:
                ...
                if isinstance(result_data, dict) and "results" in result_data:
                    ...
                    list_response = requests.get(
                        f"{self.openmemory_base_url}/api/v1/memories/?user_id={self.test_user_id}"
                    )
                ...
                if "id" in result_data:
                    self.created_memories.append(
                        {"id": result_data["id"], "source": "openmemory"}
                    )
                    return TestResult(
                        test_name="openmemory_create_memory",
                        passed=True,
    ...
    def test_openmemory_get_memory_by_id(self) -> TestResult:
        ...
        response = requests.get(
            f"{self.openmemory_base_url}/api/v1/memories/{memory_id}"
        )
    ...
    def test_openmemory_search_memories(self) -> TestResult:
        ...
        response = requests.get(
            f"{self.openmemory_base_url}/api/v1/memories/", params=params
        )
    ...
    def test_openmemory_update_memory(self) -> TestResult:
        ...
        response = requests.put(
            f"{self.openmemory_base_url}/api/v1/memories/{memory_id}", json=update_data
        )
        ...
        for memory in self.created_memories:
            try:
                if memory["source"] == "mem0":
                    ...
                    requests.delete(
                        f"{self.openmemory_base_url}/api/v1/memories/{memory['id']}"
                    )
                cleanup_count += 1
            except Exception as e:
