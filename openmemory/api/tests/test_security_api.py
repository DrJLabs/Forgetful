...
class TestEndpointAuthorization:
            ...
            try:
                if endpoint["method"] == "GET":
                    ...
                    response = await test_client.post(
                        endpoint["path"], json=endpoint["json"]
                    )
                else:
                    continue
                ...
                logger.info(
                    f"Endpoint {endpoint['method']} {endpoint['path']}: {response.status_code}"
                )

            except Exception as e:
                logger.info(
                    f"Endpoint {endpoint['method']} {endpoint['path']} test failed: {e}"
                )
    ...
    async def test_cross_user_data_access_prevention(self, test_client: AsyncClient):
        ...
        for scenario in user_scenarios:
            ...
            response = await test_client.get(
                "/api/v1/memories/", params={"user_id": scenario["target_user"]}
            )
            ...
            if response.status_code == 200:
                ...
                if items:
                    logger.info(
                        f"User {scenario['user_id']} accessing {scenario['target_user']}: {len(items)} items returned"
                    )
                else:
                    logger.info(
                        f"User {scenario['user_id']} accessing {scenario['target_user']}: No items returned (correct)"
                    )
            ...
            try:
                ...
                if method == "GET":
                    ...
                    response = await test_client.patch(
                        endpoint, json={"user_id": "test", "text": "test"}
                    )
                elif method == "HEAD":
...
class TestDataExposurePrevention:
                ...
                if response.status_code == 200:
                    ...
                    for pattern in sensitive_patterns:
                        if pattern in response_text:
                            logger.warning(
                                f"⚠ Potentially sensitive data '{pattern}' found in {endpoint}"
                            )

                            # Extract context around the sensitive data
                            start = max(0, response_text.find(pattern) - 50)
                            end = min(
                                len(response_text), response_text.find(pattern) + 50
                            )
    ...
    async def test_error_message_information_disclosure(self, test_client: AsyncClient):
        ...
        for scenario in error_scenarios:
            try:
                response = await test_client.get(
                    scenario["endpoint"], params=scenario["params"]
                )
                ...
                if response.status_code >= 400:
                    ...
                    for pattern in sensitive_error_patterns:
                        if pattern in error_text:
                            logger.warning(
                                f"⚠ Sensitive error info '{pattern}' in {scenario['endpoint']}"
                            )
                    ...
                    if response.status_code == 422:
                        try:
                            ...
                            if "detail" in error_data:
                                logger.info(
                                    f"Error detail structure: {type(error_data['detail'])}"
                                )
                        except:
                            pass

                    logger.info(
                        f"✓ Error response {response.status_code} for {scenario['endpoint']} checked"
                    )
    ...
    async def test_pagination_data_exposure(self, test_client: AsyncClient):
        ...
        for params in pagination_tests:
            try:
                response = await test_client.get(
                    "/api/v1/memories/", params={"user_id": "test", **params}
                )
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"✓ Pagination test page={params['page']}, size={params['size']}: {len(items)} items"
                    )
            ...
            try:
                ...
                for field in sensitive_metadata:
                    if field in response_text:
                        logger.warning(
                            f"⚠ Sensitive metadata '{field}' may be exposed in response"
                        )
    ...
    async def test_large_payload_protection(self, test_client: AsyncClient):
        ...
        for size in payload_sizes:
            ...
            try:
                response = await test_client.post(
                    "/api/v1/memories/",
                    json={"user_id": "test", "text": large_text, "app": "test"},
                )
                ...
                if size > 100000:  # 100KB
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"✓ Large payload {size} bytes was rejected: {response.status_code}"
                        )
            ...
            try:
                ...
                if depth > 20:
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"✓ Deep nesting {depth} was rejected: {response.status_code}"
                        )
            ...
            try:
                ...
                if size > 1000:
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"✓ Large array {size} elements was rejected: {response.status_code}"
                        )
...
class TestHTTPMethodValidation:
    ...
    async def test_method_override_protection(self, test_client: AsyncClient):
        ...
        for header in override_headers:
            try:
                response = await test_client.get(
                    "/api/v1/memories/", params={"user_id": "test"}, headers=header
                )
...
class TestAPISecurityIntegration:
        ...
        for step in security_flow:
            try:
                if step["action"] == "create_memory":
                    ...
                    response = await test_client.get(
                        "/api/v1/memories/", params={"user_id": "security_test"}
                    )
                elif step["action"] == "get_stats":
                    response = await test_client.get(
                        "/api/v1/stats/", params={"user_id": "security_test"}
                    )
                else:
                    continue
    ...
    async def test_concurrent_security_validation(self, test_client: AsyncClient):
        ...
        logger.info(
            f"Concurrent security test: {successful_requests}/{concurrent_count} successful"
        )
