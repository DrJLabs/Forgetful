    ...
    def test_schema_reference_resolution(self, openapi_schema):
        ...
        for ref in refs:
            if ref.startswith("#/components/schemas/"):
                ...
                assert schema_name in schemas, f"Referenced schema not found: {schema_name}"
        ...
        for schema_name, schema_def in pagination_schemas:
            if "properties" in schema_def:
                ...
                for field in required_pagination_fields:
                    assert field in properties, f"Missing pagination field '{field}' in {schema_name}"
            ...
            if schema_name in schemas:
                ...
                if "properties" in schema:
                    ...
                    for field in required_fields:
                        assert field in properties, f"Missing required field '{field}' in {schema_name}"
    ...
    def test_http_method_stability(self, openapi_schema):
        ...
        for endpoint, method in critical_operations:
            if endpoint in paths:
                assert method in paths[endpoint], f"Missing method {method} for {endpoint}"
