...
class TestRateLimitingMechanisms:
    ...
    def test_rate_limit_calculation_logic(self):
        ...
        rate_limits = [
            {"requests": 10, "window": 60, "expected_interval": 6.0},  # 10 req/min = 1 req/6s
            {"requests": 100, "window": 3600, "expected_interval": 36.0},  # 100 req/hour = 1 req/36s
            {"requests": 1000, "window": 86400, "expected_interval": 86.4},  # 1000 req/day = 1 req/86.4s
        ...
        for limit in rate_limits:
            ...
            logger.info(f"Rate limit test: {limit['requests']} req/{limit['window']}s = 1 req/{expected_interval}s")
    ...
    def test_burst_detection_logic(self):
        ...
        logger.info(f"Burst detection: {requests_in_window} requests in {window_seconds}s window")
    ...
    def test_sliding_window_logic(self):
        ...
        logger.info(f"Sliding window: {len(recent_requests)} requests in last {window_size}s")
...
class TestAPIRateLimiting:
    ...
    async def test_memory_list_rate_limiting(self, test_client: AsyncClient, test_user):
        ...
        logger.info(f"Rate limiting test: {len(successful_responses)} successful, {len(rate_limited_responses)} rate limited in {total_time:.2f}s")
        ...
        assert all(r is None or r.status_code in [200, 429, 500, 503] for r in responses)
    ...
    async def test_memory_creation_rate_limiting(self, test_client: AsyncClient):
        ...
        rate_limited_responses = [r for r in responses if r and r.status_code == 429]
        error_responses = [r for r in responses if r and r.status_code >= 400 and r.status_code != 429]
        
        logger.info(f"Memory creation rate limiting: {len(successful_responses)} successful, {len(rate_limited_responses)} rate limited, {len(error_responses)} errors")
        
        # System should handle rapid requests gracefully
        assert len(responses) == request_count
        assert all(r is None or r.status_code in [200, 201, 400, 404, 422, 429, 500, 503] for r in responses)

    @pytest.mark.asyncio
    async def test_concurrent_request_limiting(self, test_client: AsyncClient, test_user):
        ...
        logger.info(f"Concurrent requests: {len(successful_responses)} successful, {len(failed_responses)} failed in {end_time - start_time:.2f}s")
...
class TestBruteForceProtection:
    ...
    @pytest.mark.asyncio
    async def test_authentication_brute_force_protection(self, test_client: AsyncClient):
        ...
        for i in range(brute_force_attempts):
            try:
                ...
                response = await test_client.get(
                    "/api/v1/memories/",
                    params={"user_id": f"invalid_user_{i}"}
                )
        ...
        logger.info(f"Brute force protection: {successful_attempts} successful, {not_found_attempts} not found, {rate_limited_attempts} rate limited")
    ...
    async def test_password_brute_force_simulation(self, test_client: AsyncClient):
        ...
        common_passwords = [
            ...
            "password123", "admin123", "qwerty", "abc123", "password1"
        ]
        ...
        for password in common_passwords:
            try:
                ...
                response = await test_client.get(
                    "/api/v1/memories/",
                    params={"user_id": f"admin_{password}"}
                )
                ...
                logger.info(f"Password brute force attempt with '{password}' failed: {e}")
                brute_force_results.append({
                    "password": password,
                    "status": None,
                    "success": False
        ...
        logger.info(f"Password brute force simulation: {successful_attempts}/{len(common_passwords)} successful attempts")
    ...
    async def test_account_lockout_simulation(self, test_client: AsyncClient):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if response.status_code == 423:
                    logger.info(f"Account lockout detected after {attempt + 1} attempts")
                    break
        ...
        logger.info(f"Account lockout simulation: {locked_attempts} lockout responses in {len(lockout_results)} attempts")
...
class TestDDoSProtection:
    ...
    async def test_request_flood_protection(self, test_client: AsyncClient):
        ...
        async def flood_request(request_id):
            try:
                response = await test_client.get(
                    "/api/v1/memories/",
                    params={"user_id": f"flood_user_{request_id}"}
                )
                return {
                    "id": request_id,
                    "status": response.status_code,
        ...
        successful_requests = sum(1 for r in flood_results if isinstance(r, dict) and r.get("success"))
        failed_requests = len(flood_results) - successful_requests
        requests_per_second = flood_count / flood_duration
        
        logger.info(f"Request flood: {successful_requests} successful, {failed_requests} failed, {requests_per_second:.2f} req/s")
    ...
    async def test_resource_exhaustion_protection(self, test_client: AsyncClient):
        ...
        logger.info(f"Resource exhaustion test: {accepted_payloads} accepted, {rejected_payloads} rejected large payloads")
    ...
    async def test_connection_flood_protection(self, test_client: AsyncClient):
        ...
        async def test_connection(conn_id):
            try:
                response = await test_client.get(
                    "/api/v1/stats/",
                    params={"user_id": f"conn_test_{conn_id}"}
                )
                return {
                    "id": conn_id,
                    "status": response.status_code,
                    "connected": response.status_code in [200, 404]  # Connected successfully
                }
            except Exception as e:
                return {
                    "id": conn_id,
        ...
        successful_connections = sum(1 for r in connection_results if isinstance(r, dict) and r.get("connected"))
        failed_connections = len(connection_results) - successful_connections
        
        logger.info(f"Connection flood: {successful_connections} successful, {failed_connections} failed in {connection_duration:.2f}s")
...
class TestRateLimitingEdgeCases:
    ...
    def test_rate_limit_reset_logic(self):
        ...
        if current_time >= window_end:
            ...
            logger.info(f"Rate limit window active, {time_remaining.total_seconds():.2f}s remaining")
    ...
    def test_rate_limit_bypass_attempts(self):
        ...
        bypass_attempts = [
            ...
            {"technique": "User-Agent rotation", "headers": {"User-Agent": "Mozilla/5.0 (Different Browser)"}},
            {"technique": "Referrer spoofing", "headers": {"Referer": "https://legitimate-site.com"}},
            {"technique": "Header manipulation", "headers": {"X-Real-IP": "192.168.1.1"}},
            {"technique": "Session rotation", "headers": {"X-Session-ID": "new-session-123"}},
        ]
