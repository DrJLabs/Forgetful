...
class TestMemoriesRouter:
    ...
    async def test_list_memories_success(
        self, test_client: AsyncClient, test_user, mock_memory_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_list_memories_with_search(
        self, test_client: AsyncClient, test_user, mock_memory_client
    ):
        """Test memory listing with search query"""
        response = await test_client.get(
            "/api/v1/memories/",
            params={"user_id": test_user.user_id, "search_query": "test"},
        )
    ...
    async def test_list_memories_with_pagination(
        ...
        response = await test_client.get(
            "/api/v1/memories/",
            params={"user_id": test_user.user_id, "page": 1, "size": 10},
        )
    ...
    @pytest.mark.asyncio
    async def test_create_memory_client_error(
        self, test_client: AsyncClient, test_user, mock_memory_client
    ):
    ...
    async def test_get_memory_success(
        self, test_client: AsyncClient, mock_memory_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_memory_client_error(
        self, test_client: AsyncClient, mock_memory_client
    ):
    ...
    async def test_search_memories_success(
        ...
        search_data = {"user_id": test_user.user_id, "query": "test query", "limit": 10}

        response = await test_client.post("/api/v1/memories/search", json=search_data)
    ...
    async def test_get_categories_empty(self, test_client: AsyncClient, test_user):
        ...
        response = await test_client.get(
            "/api/v1/memories/categories", params={"user_id": test_user.user_id}
        )
...
class TestAppsRouter:
    ...
    async def test_list_apps_with_pagination(self, test_client: AsyncClient, test_app):
        ...
        response = await test_client.get(
            "/api/v1/apps/", params={"page": 1, "page_size": 5}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_app_memories_with_pagination(
        self, test_client: AsyncClient, test_app
    ):
        """Test getting app memories with pagination"""
        response = await test_client.get(
            f"/api/v1/apps/{test_app.id}/memories", params={"page": 1, "page_size": 10}
        )
...
class TestStatsRouter:
    ...
    async def test_get_profile_success(
        ...
        response = await test_client.get(
            "/api/v1/stats/", params={"user_id": test_user.user_id}
        )
    ...
    async def test_get_profile_new_user(
        ...
        response = await test_client.get(
            "/api/v1/stats/", params={"user_id": new_user_id}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_profile_memory_client_error(
        self, test_client: AsyncClient, test_user, mock_memory_client
    ):
...
class TestConfigRouter:
    ...
    async def test_update_openmemory_configuration(self, test_client: AsyncClient):
        ...
        config_data = {"enable_logging": True, "max_memory_size": 1000}

        response = await test_client.put("/api/v1/config/openmemory", json=config_data)
...
class TestValidation:
    ...
    async def test_memory_creation_validation(self, test_client: AsyncClient):
        ...
        # Test empty text
        response = await test_client.post(
            "/api/v1/memories/", json={"user_id": "test", "text": "", "app": "test"}
        )
    ...
    async def test_pagination_validation(self, test_client: AsyncClient, test_user):
        ...
        response = await test_client.get(
            "/api/v1/memories/", params={"user_id": test_user.user_id, "page": -1}
        )
        ...
        # Test zero page size
        response = await test_client.get(
            "/api/v1/memories/", params={"user_id": test_user.user_id, "size": 0}
        )
