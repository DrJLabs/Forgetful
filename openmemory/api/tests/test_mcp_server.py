...
from app.mcp_server import (
    mcp, add_memories, search_memory, list_memories, delete_memories,
    user_id_var, client_name_var, get_memory_client_safe, mcp_router
)
...
class TestMCPServerFunctionality:
    ...
    async def test_add_memories_client_unavailable(self):
        ...
        with patch('app.mcp_server.get_memory_client_safe') as mock_client:
            ...
            assert result == "Error: Memory system is currently unavailable. Please try again later."
...
class TestMCPPerformance:
    ...
    async def test_concurrent_memory_operations(self):
        ...
        with patch('app.mcp_server.get_memory_client_safe') as mock_client:
            ...
            for result in results:
                assert isinstance(result, str)
                assert "Error" not in result or "Successfully" in result or "Found" in result
    ...
    async def test_memory_operation_timeout(self):
        ...
        with patch('app.mcp_server.get_memory_client_safe') as mock_client:
            ...
            # Test with timeout
            try:
                result = await asyncio.wait_for(add_memories("slow memory"), timeout=2.0)
