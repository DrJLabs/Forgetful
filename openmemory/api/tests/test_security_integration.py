...
class TestMultiLayerSecurity:
    ...
    async def test_comprehensive_security_audit(self, test_client: AsyncClient):
        ...
        input_tests = [
            ...
            {"name": "xss_script", "text": "<script>alert('xss')</script>", "should_pass": False},
            {"name": "sql_injection", "text": "'; DROP TABLE memories; --", "should_pass": False},
        ]
                ...
                if test["should_pass"]:
                    ...
                    if response.status_code in [400, 422]:
                        ...
                        if response.status_code in [200, 201]:
                            ...
                            if "<script>" not in response_text and "drop table" not in response_text:
                                security_audit["input_validation"]["passed"] += 1
        ...
        header_tests = [
            {"name": "cors_header", "header": "access-control-allow-origin", "required": True},
            {"name": "content_type_options", "header": "x-content-type-options", "required": False},
        ...
        for layer, results in security_audit.items():
            ...
            if total_tests > 0:
                ...
                logger.info(f"{layer.upper()}: {results['passed']}/{total_tests} passed ({pass_rate:.1f}%)")
                
                for test in results["tests"]:
                    status_icon = "✓" if test["result"] == "PASS" else "✗" if test["result"] == "FAIL" else "?"
        ...
        if total_tests > 0:
            ...
            logger.info(f"OVERALL SECURITY SCORE: {security_score:.1f}% ({total_passed}/{total_tests})")
            
            # Assert minimum security score
            assert security_score >= 70, f"Security score {security_score:.1f}% is below minimum threshold of 70%"
    ...
    async def test_security_policy_enforcement(self, test_client: AsyncClient):
        ...
        security_policies = {
            "data_access": {
                ...
                "test_cases": [
                    ...
                    {"user": "admin", "target": "user1", "should_allow": True},  # Admin can access all
                ]
            },
            "input_sanitization": {
        ...
        for i, case in enumerate(security_policies["input_sanitization"]["test_cases"]):
            try:
                response = await test_client.post(
                    "/api/v1/memories/",
                    json={"user_id": f"sanitization_test_{i}", "text": case["input"], "app": "test"}
                )
                ...
                if case["should_sanitize"]:
                    ...
                    if response.status_code in [400, 422]:
                        ...
                        data = response.json()
                        response_text = json.dumps(data).lower()
                        policy_passed = "<script>" not in response_text and "drop table" not in response_text
                    else:
                        policy_passed = False
                else:
        ...
        for i, case in enumerate(security_policies["rate_limiting"]["test_cases"]):
            try:
                ...
                for j in range(case["requests"]):
                    response = await test_client.get(
                        "/api/v1/memories/",
                        params={"user_id": f"rate_limit_{i}_{j}"}
                    )
        ...
        for policy_name, result in policy_results.items():
            ...
            if result["passed"]:
                ...
                logger.info(f"✗ {policy_name}: FAIL (expected: {result['expected']}, actual: {result['actual']})")
        
        policy_compliance = (passed_policies / total_policies) * 100 if total_policies > 0 else 0
        logger.info(f"POLICY COMPLIANCE: {policy_compliance:.1f}% ({passed_policies}/{total_policies})")
        
        # Assert minimum policy compliance
        assert policy_compliance >= 80, f"Policy compliance {policy_compliance:.1f}% is below minimum threshold of 80%"
    ...
    async def test_cross_component_security(self, test_client: AsyncClient):
            ...
            for component_name, endpoint in components.items():
                try:
                    ...
                    if test["name"] == "consistent_auth":
                        response = await test_client.get(endpoint, params={"user_id": test["test_user"]})
                    ...
                    # Test validation consistency
                        ...
                        test_results[component_name] = {
                            "status_code": response.status_code,
                            "validation_handled": response.status_code in [400, 404, 422]
                        }
        ...
        for test_name, results in component_results.items():
            ...
            if test_name == "consistent_auth":
                ...
                content_type_headers = [r.get("content_type_header", False) for r in results.values()]
                
                cors_consistency = all(cors_headers) or all(not h for h in cors_headers)
                content_type_consistency = all(content_type_headers) or all(not h for h in content_type_headers)
                
                logger.info(f"  CORS header consistency: {'✓' if cors_consistency else '✗'}")
                logger.info(f"  Content-Type header consistency: {'✓' if content_type_consistency else '✗'}")
            
            elif test_name == "consistent_validation":
                validation_handling = [r.get("validation_handled", False) for r in results.values()]
                consistency = all(validation_handling) or all(not h for h in validation_handling)
...
class TestSecurityRegressionPrevention:
    ...
    async def test_known_vulnerability_patterns(self, test_client: AsyncClient):
        ...
        for vuln_type, patterns in vulnerability_patterns.items():
            ...
            for pattern in patterns:
                try:
                    ...
                    contexts = [
                        {"type": "url_param", "endpoint": "/api/v1/memories/", "params": {"user_id": pattern}},
                        {"type": "post_body", "endpoint": "/api/v1/memories/", "json": {"user_id": "test", "text": pattern, "app": "test"}},
                        {"type": "search_query", "endpoint": "/api/v1/memories/", "params": {"user_id": "test", "search_query": pattern}},
                    ]
                    
                    for context in contexts:
                        try:
                            if context["type"] == "url_param" or context["type"] == "search_query":
                                response = await test_client.get(context["endpoint"], params=context["params"])
                            elif context["type"] == "post_body":
                                response = await test_client.post(context["endpoint"], json=context["json"])
                            else:
                                continue
                            ...
                            if response.status_code == 200:
                                ...
                                if any(sign in response_text for sign in ["root:", "bin/", "etc/passwd"]):
                                    vulnerable = True
        ...
        for vuln_type, results in vulnerability_results.items():
            ...
            logger.info(f"{vuln_type.upper()}: {handled_count}/{len(results)} handled, {vulnerable_count} vulnerable")
            
            # Log vulnerable patterns
            for result in results:
                if result["vulnerable"]:
                    logger.warning(f"  ⚠ VULNERABLE: {result['pattern']} in {result['context']} (status: {result['status_code']})")
        ...
        if total_tests > 0:
            ...
            logger.info(f"OVERALL: {total_handled}/{total_tests} handled ({handling_rate:.1f}%), {total_vulnerable} vulnerable ({vulnerability_rate:.1f}%)")
            
            # Assert security thresholds
            assert vulnerability_rate < 5, f"Vulnerability rate {vulnerability_rate:.1f}% is above acceptable threshold of 5%"
            assert handling_rate > 90, f"Handling rate {handling_rate:.1f}% is below minimum threshold of 90%"
    ...
    async def test_security_configuration_validation(self, test_client: AsyncClient):
            ...
            try:
                if test["test"] == "cors_headers":
                    ...
                    if response.status_code >= 400:
                        ...
                        if any(leak in error_text for leak in ["traceback", "stack", "debug", "internal"]):
                            error_safe = False
        ...
        assert len(failed_configs) == 0, f"Failed security configurations: {failed_configs}"
...
class TestSecurityMonitoring:
    ...
    async def test_security_event_logging(self, test_client: AsyncClient):
        ...
        security_events = [
            ...
            {"event": "xss_attempt", "json": {"user_id": "test", "text": "<script>alert('xss')</script>", "app": "test"}},
            {"event": "large_payload", "json": {"user_id": "test", "text": "A" * 10000, "app": "test"}},
        ]
        ...
        for event in security_events:
            try:
                if "params" in event:
                    response = await test_client.get("/api/v1/memories/", params=event["params"])
                elif "json" in event:
                    response = await test_client.post("/api/v1/memories/", json=event["json"])
                else:
                    continue
                ...
                # Log security event
                logger.warning(f"Security event: {event['event']} -> {response.status_code}")
        ...
        assert handled_events == total_events, f"Not all security events were properly handled: {handled_events}/{total_events}"
        ...
        for test in metrics_tests:
            if test["name"] == "request_count":
                ...
                for i in range(test["requests"]):
                    await test_client.get("/api/v1/memories/", params={"user_id": f"metrics_test_{i}"})
                ...
                for i in range(test["errors"]):
                    await test_client.get("/api/v1/nonexistent/", params={"user_id": f"error_test_{i}"})
