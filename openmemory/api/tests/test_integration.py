...
class TestMemoryLifecycle:
    ...
    @pytest.mark.asyncio
    async def test_complete_memory_workflow(self, test_client: AsyncClient, test_db_session, mock_memory_client):
    ...
    @pytest.mark.asyncio
    async def test_memory_with_categories_integration(self, test_client: AsyncClient, test_db_session, mock_memory_client):
    ...
    @pytest.mark.asyncio
    async def test_app_memory_relationship_integration(self, test_client: AsyncClient, test_db_session, mock_memory_client):
...
class TestDatabaseIntegration:
    ...
    @pytest.mark.asyncio
    async def test_database_transaction_rollback(self, test_client: AsyncClient, test_db_session):
        ...
        existing_user = test_db_session.query(User).filter(User.user_id == "transaction_user").first()
        assert existing_user is not None
        assert existing_user.name == "Test User"
    
    @pytest.mark.asyncio
    async def test_database_connection_handling(self, test_client: AsyncClient, test_db_session):
    ...
    @pytest.mark.asyncio
    async def test_database_constraints(self, test_client: AsyncClient, test_db_session):
...
class TestMemoryClientIntegration:
    ...
    @pytest.mark.asyncio
    async def test_memory_client_error_handling(self, test_client: AsyncClient, test_db_session):
    ...
    @pytest.mark.asyncio
    async def test_memory_client_timeout_handling(self, test_client: AsyncClient, test_db_session):
    ...
    @pytest.mark.asyncio
    async def test_memory_client_reconnection(self, test_client: AsyncClient, test_db_session):
...
class TestConfigurationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_configuration_persistence(self, test_client: AsyncClient, test_db_session):
    ...
    @pytest.mark.asyncio
    async def test_openmemory_config_isolation(self, test_client: AsyncClient, test_db_session):
        ...
        response = await test_client.put("/api/v1/config/openmemory", json=openmemory_config)
        assert response.status_code == status.HTTP_200_OK
...
class TestPaginationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_memory_pagination_consistency(self, test_client: AsyncClient, test_db_session, mock_memory_client):
        """Test pagination consistency across memory endpoints"""
        # Mock large dataset
        mock_memories = [
            {"id": f"mem_{i}", "memory": f"Memory {i}", "created_at": datetime.now(UTC).isoformat()}
            for i in range(25)
        ]
    ...
    @pytest.mark.asyncio
    async def test_app_pagination_consistency(self, test_client: AsyncClient, test_db_session):
...
class TestErrorPropagation:
    ...
    @pytest.mark.asyncio
    async def test_database_error_propagation(self, test_client: AsyncClient, test_db_session):
...
class TestConcurrencyIntegration:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_memory_creation(self, test_client: AsyncClient, test_db_session, mock_memory_client):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_app_creation(self, test_client: AsyncClient, test_db_session, mock_memory_client):
        """Test concurrent app creation through memory creation"""
        # Create memories with different apps concurrently
        tasks = []
        for i in range(3):
            memory_data = TestDataFactory.create_memory_data("concurrent_user", f"concurrent_app_{i}")
            task = test_client.post("/api/v1/memories/", json=memory_data)
            tasks.append(task)
