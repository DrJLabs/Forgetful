...
class TestTransactionRollback:
    ...
    def test_session_rollback_on_exception(self, postgres_test_session, test_user_factory):
        ...
        try:
            ...
            assert postgres_test_session.query(User).filter_by(user_id="rollback_test").count() == 1
    ...
    def test_manual_rollback(self, transaction_test_session, test_user_factory):
        ...
        try:
            ...
            transaction_test_session.execute(
                text("INSERT INTO users (id, user_id, name, created_at) VALUES (:id, :user_id, :name, :created_at)"),
                {
                    "id": str(user.id),
    ...
    def test_concurrent_transactions(self, concurrent_sessions, test_user_factory):
        ...
        try:
            ...
            session2.query(User).filter_by(user_id="concurrent_test_1").update({"name": "Updated by session2"})
            
            # Try to read from session3 - should see original value
            user_in_session3 = session3.query(User).filter_by(user_id="concurrent_test_1").first()
    ...
    def test_savepoint_rollback(self, postgres_test_session, test_user_factory):
        ...
        try:
            ...
            assert postgres_test_session.query(User).filter_by(user_id="savepoint_test_1").count() == 1
            assert postgres_test_session.query(User).filter_by(user_id="savepoint_test_2").count() == 1
            ...
            assert postgres_test_session.query(User).filter_by(user_id="savepoint_test_1").count() == 1
            assert postgres_test_session.query(User).filter_by(user_id="savepoint_test_2").count() == 0
...
class TestDatabaseIntegrity:
    ...
    def test_foreign_key_constraints(self, postgres_test_session, test_user_factory, test_app_factory):
    ...
    def test_cascade_deletes(self, postgres_test_session, test_user_factory, test_app_factory):
        ...
        remaining_apps = postgres_test_session.query(App).filter_by(user_id=user.user_id).count()
...
class TestPerformanceMonitoring:
    ...
    def test_query_performance(self, postgres_test_session, test_user_factory, performance_monitor):
        ...
        result = postgres_test_session.query(User).filter(User.user_id.like("perf_test_%")).all()
        end_time = time.time()
    ...
    def test_memory_usage_monitoring(self, postgres_test_session, test_user_factory):
        ...
        batch_size = 1000
        users = [test_user_factory(user_id=f"memory_test_{i}") for i in range(batch_size)]
        ...
        count = postgres_test_session.query(User).filter(User.user_id.like("memory_test_%")).count()
...
class TestConcurrentAccess:
    ...
    def test_concurrent_reads(self, docker_postgres_engine, test_user_factory):
        ...
        with docker_postgres_engine.connect() as conn:
            ...
            users = [test_user_factory(user_id=f"concurrent_read_{i}") for i in range(10)]
        ...
        def read_operation(thread_id):
            ...
            with docker_postgres_engine.connect() as conn:
                ...
                result = session.query(User).filter(User.user_id.like("concurrent_read_%")).all()
                session.close()
                return len(result)
    ...
    def test_concurrent_writes(self, docker_postgres_engine, test_user_factory):
        ...
        with docker_postgres_engine.connect() as conn:
            ...
            count = session.query(User).filter(User.user_id.like("concurrent_write_%")).count()
            assert count == 3
            session.close()
    ...
    def test_deadlock_detection(self, concurrent_sessions, test_user_factory):
        ...
        try:
            ...
            session1.query(User).filter_by(user_id="deadlock_test_1").with_for_update().first()
            
            # Session2 locks user2
            session2.query(User).filter_by(user_id="deadlock_test_2").with_for_update().first()
            
            # Now try to access in reverse order (potential deadlock)
            # Session1 tries to lock user2
            session1.query(User).filter_by(user_id="deadlock_test_2").with_for_update().first()
            
            # Session2 tries to lock user1
            session2.query(User).filter_by(user_id="deadlock_test_1").with_for_update().first()
...
class TestDatabaseIntegrationScenarios:
    ...
    def test_complete_memory_lifecycle(self, postgres_test_session, test_user_factory, test_app_factory, test_memory_factory):
        ...
        # Read memory
        retrieved_memory = postgres_test_session.query(Memory).filter_by(id=memory.id).first()
        ...
        # Verify update
        updated_memory = postgres_test_session.query(Memory).filter_by(id=memory.id).first()
        ...
        # Verify deletion
        deleted_memory = postgres_test_session.query(Memory).filter_by(id=memory.id).first()
        assert deleted_memory is None
    
    def test_bulk_operations_performance(self, postgres_test_session, test_user_factory, performance_monitor):
        ...
        # Bulk delete
        start_time = time.time()
        postgres_test_session.query(User).filter(User.user_id.like("bulk_%")).delete(synchronize_session=False)
        postgres_test_session.commit()
