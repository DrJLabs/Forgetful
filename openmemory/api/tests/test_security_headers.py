    ...
    async def test_hsts_header(self, test_client: AsyncClient):
        ...
        if hsts_header:
            ...
            if "max-age=" in hsts_header:
                ...
                max_age_part = [
                    part for part in hsts_header.split(";") if "max-age=" in part
                ][0]
                ...
                if max_age >= min_age:
                    ...
                    logger.warning(
                        f"⚠ HSTS max-age may be too short: {max_age} seconds"
                    )
        ...
        if server_header:
            ...
            for info in sensitive_info:
                if info.lower() in server_header.lower():
                    logger.warning(
                        f"⚠ Server header may disclose sensitive info: {info}"
                    )
        else:
...
class TestSecurityHeadersIntegration:
    ...
    async def test_security_headers_consistency(self, test_client: AsyncClient):
        ...
        for i in range(3):
            response = await test_client.get(
                "/api/v1/memories/", params={"user_id": f"test_{i}"}
            )
    ...
    async def test_post_request_security_headers(self, test_client: AsyncClient):
        ...
        for header in security_headers:
            if header in response.headers:
                logger.info(
                    f"POST response header {header}: {response.headers[header]}"
                )
