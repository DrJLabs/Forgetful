...
from mem0.memory.timezone_utils import safe_datetime_now, safe_datetime_diff, get_memory_age_days
...
class EnhancedConfidenceScorer:
    ...
    def calculate_confidence(
        ...
        final_confidence = min(
            ...
            temporal_factor * historical_factor * content_quality,
            1.0
        )
    ...
    def _get_indicator_keywords(self, indicator: str) -> List[str]:
        ...
        """
        keywords = {
            'code_block': ['def ', 'function', 'class ', 'import ', 'from ', '{', '}', '()', '[]'],
            'file_path': ['.py', '.js', '.ts', '.java', '.cpp', '.c', '.h', '/', '\\'],
            'error_message': ['error', 'exception', 'failed', 'crash', 'traceback', 'stack trace'],
            'solution_steps': ['step', 'first', 'then', 'next', 'finally', 'solution', 'fix'],
            'configuration': ['config', 'setting', 'parameter', 'option', 'environment', 'env'],
            'performance_data': ['ms', 'seconds', 'memory', 'cpu', 'benchmark', 'optimization']
        }
    ...
    def _calculate_retrieval_boost(self, metadata: Dict[str, Any], context: Dict[str, Any]) -> float:
        ...
        if access_count > 0:
            frequency_boost = min(math.log(access_count + 1) * 0.02, self.retrieval_factors['frequency_boost'])
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.accuracy_metrics,
            'accuracy': accuracy,
            'avg_confidence': sum(h['confidence'] for h in self.scoring_history) / len(self.scoring_history) if self.scoring_history else 0.0,
        ...
        if accuracy < 0.7:
            ...
            for category in self.category_weights:
                self.category_weights[category] = max(
                    self.category_weights[category] * 0.95,
                    0.5
                )
        elif accuracy > 0.9:
            ...
            for category in self.category_weights:
                self.category_weights[category] = min(
                    self.category_weights[category] * 1.02,
                    1.0
                )
