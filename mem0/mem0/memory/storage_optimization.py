...
class IntelligentStorageManager:
    ...
    def _context_aware_purge(self, memories: List[Dict[str, Any]], count: int) -> List[Dict[str, Any]]:
        ...
        if remaining_count > 0:
            remaining_memories = [
                m for m in memories if m not in memories_to_purge
            ]
    ...
    def _purge_category_memories(
        ...
        for score, memory in scored_memories:
            ...
            if created_at:
                try:
                    created_time = datetime.fromisoformat(created_at.replace('Z', '+00:00'))
                    if current_time - created_time.replace(tzinfo=None) > max_age:
    ...
    def _calculate_performance_impact(
        ...
        purged_memories: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        high_access_purged = sum(
            1 for memory in purged_memories
    ...
    def _perform_autonomous_optimization(
        ...
        if urgency == 'emergency':
            ...
            target_reduction = min(
                self.autonomous_settings['max_auto_purge_percent'],
                0.2
            )
    ...
    def _update_learning_data(self, result: Dict[str, Any]):
        ...
        if size_saved > 0:
            ...
            self.performance_tracking['average_performance_gain'] = (
                (current_avg * (total_opts - 1) + performance_gain) / total_opts
            )
    ...
    def provide_feedback(self, optimization_id: str, satisfaction_score: float):
        ...
        if total_opts > 0:
            self.performance_tracking['user_satisfaction_score'] = (
                (current_score * (total_opts - 1) + satisfaction_score) / total_opts
            )
        else:
