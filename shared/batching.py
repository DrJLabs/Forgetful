...
class BatchMetrics:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            avg_processing_time = self.processing_time_total / max(
                self.batches_processed, 1
            )
            success_rate = (self.requests_processed - self.requests_failed) / max(
...
class BatchProcessor:
    ...
    async def start(self):
        ...
        logger.info(
            f"Batch processor '{self.name}' started with {self.config.parallel_workers} workers"
        )
    ...
    async def add_request(self, request: BatchRequest) -> Any:
        ...
        request.callback = (
            future  # Fix: Assign future object, not future.set_result method
        )
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    ...
                    logger.warning(
                        f"Batch processing attempt {attempt + 1} failed in '{self.name}': {e}"
                    )
                else:
                    logger.error(
                        f"Batch processing failed after {self.config.max_retries} retries in '{self.name}': {e}"
                    )
...
class VectorSearchBatcher:
    ...
    def __init__(self):
        ...
        self.processor = BatchProcessor(
            "vector_search", self._process_vector_searches, self.config
        )
    ...
    async def _process_vector_searches(self, batch: List[BatchRequest]) -> List[Any]:
        ...
        try:
            ...
            if global_pool_manager.postgres_pool:
                async with global_pool_manager.postgres_pool.acquire() as conn:
                    for request in batch:
                        search_result = await self._execute_vector_search(
                            conn, request.data
                        )
...
class GraphQueryBatcher:
    ...
    async def add_graph_query(
        ...
        request = BatchRequest(
            ...
            data={"user_id": user_id, "query": query, "parameters": parameters or {}},
            priority=priority,
            timeout=15.0,
        )
        ...
        try:
            ...
            if global_pool_manager.neo4j_pool:
                async with global_pool_manager.neo4j_pool.session() as session:
                    for request in batch:
                        query_result = await self._execute_graph_query(
                ...
                for request in batch:
                    results.append(
                        [{"node": f"mock_{request.id}", "relationship": "RELATED_TO"}]
                    )
    ...
    def decorator(func):
        ...
        async def wrapper(
            ...
            **kwargs,
            ...
            # Use batching manager
            result = (
                await global_batching_manager.vector_search_batcher.add_vector_search(
