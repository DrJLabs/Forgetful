...
class OpenAIConfig(BaseConfig):
    ...
    # API settings
    OPENAI_REQUEST_TIMEOUT: int = Field(default=30, description="Request timeout in seconds")
...
class AppConfig(BaseConfig):
    ...
    APP_API_KEY: Optional[str] = Field(None, description="Optional API key for authentication")
...
class ServiceUrlConfig(BaseConfig):
    ...
    MEM0_API_URL: str = Field(default="http://localhost:8000", description="mem0 API URL")
    OPENMEMORY_API_URL: str = Field(default="http://localhost:8765", description="OpenMemory API URL")
    OPENMEMORY_UI_URL: str = Field(default="http://localhost:3000", description="OpenMemory UI URL")
    
    # External service URLs
    REDIS_URL: Optional[str] = Field(None, description="Redis URL for caching")
    ELASTICSEARCH_URL: Optional[str] = Field(None, description="Elasticsearch URL for search")
...
class FrontendConfig(BaseConfig):
    ...
    NEXT_PUBLIC_API_URL: str = Field(default="http://localhost:8765", description="Public API URL")
    NEXT_PUBLIC_USER_ID: str = Field(default="default_user", description="Public user ID")
    NEXT_PUBLIC_ENVIRONMENT: str = Field(default="development", description="Public environment")
...
class DockerConfig(BaseConfig):
    ...
    COMPOSE_PROJECT_NAME: str = Field(default="mem0-stack", description="Docker Compose project name")
    
    # Legacy compatibility variables
    POSTGRES_USER: Optional[str] = Field(None, description="Legacy PostgreSQL user")
    POSTGRES_PASSWORD: Optional[str] = Field(None, description="Legacy PostgreSQL password")
...
class Config(DatabaseConfig, Neo4jConfig, OpenAIConfig, AppConfig, ServiceUrlConfig, FrontendConfig, DockerConfig):
    ...
    def validate_all(self) -> List[str]:
        ...
        try:
            ...
            # Test service URLs
            for url_field in ['MEM0_API_URL', 'OPENMEMORY_API_URL', 'OPENMEMORY_UI_URL']:
