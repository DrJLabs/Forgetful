...
class OptimizedPostgreSQLPool:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"PostgreSQL pool initialized: min={self.config.postgres_min_size}, max={self.config.postgres_max_size}"
            )
    ...
    async def _pre_warm_connections(self):
        ...
        try:
            ...
            logger.info(
                f"Pre-warmed {successful}/{self.config.postgres_min_size} PostgreSQL connections"
            )
    ...
    async def _health_check_loop(self):
        ...
        while True:
            try:
                ...
                logger.debug(
                    f"PostgreSQL health check: {is_healthy}, time: {health_check_time:.3f}s"
                )
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.driver = AsyncGraphDatabase.driver(
                ...
                auth=(
                    self.neo4j_config.NEO4J_USERNAME,
                    self.neo4j_config.NEO4J_PASSWORD,
                ),
            ...
            logger.info(
                f"Neo4j pool initialized: max_size={self.config.neo4j_max_size}"
            )
    ...
    async def _health_check_loop(self):
        ...
        while True:
            try:
                ...
                logger.debug(
                    f"Neo4j health check: {is_healthy}, time: {health_check_time:.3f}s"
                )
...
class OptimizedRedisPool:
    ...
    def __init__(
        self, config: ConnectionPoolConfig, redis_url: str = "redis://localhost:6379"
    ):
        self.config = config
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"Redis pool initialized: max_size={self.config.redis_max_size}"
            )
    ...
    async def _health_check_loop(self):
        ...
        while True:
            try:
                ...
                logger.debug(
                    f"Redis health check: {is_healthy}, time: {health_check_time:.3f}s"
                )
